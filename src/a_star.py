...
def euclidean_distance(point1: Tuple[int, int], point2: Tuple[int, int]) -> float:
    ...
    :param pont2: second point as a tuple
...
def a_star(map: np.ndarray, start: Tuple[int, int], target: Tuple[int, int], 
           excludeds: List[Tuple[int, int]]) -> List[Tuple[int, int]]:
    """
    :param map: the map of the game
    :param start: the starting point as a tuple
    :param target: the target point as a tuble
    ...
    #fix the euristic function in base of the agent's movement
    h = euclidean_distance
    #h = diagonal_distance
    
    #initialize the NOT visited list: it contains the nodes that have to be visite
    not_visited = []
    ...
    while not_visited:
        ...
        neighbors = get_neighbors(map, current_node, excludeds)
